...
import ClientBoardTemplate from "../templates/ClientBoardTemplate";
...
export default function CreateBoardModal({ isOpen, onClose, onSubmit }) {
  ...
  const handleTemplateSelect = async (templateData) => {
    ...
    try {
      ...
      setFormData({ title: '', description: '', color: '#0073EA', visibility: 'private' }); // Reset form data
      onClose(); // Close the modal after successful submission from template
    } catch (error) {
      ...
      <DialogContent className="sm:max-w-2xl max-h-[80vh] overflow-y-auto">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-2">
            ...
            <TabsTrigger value="template">Client Template</TabsTrigger>
          </TabsList>
          
          <TabsContent value="blank" className="space-y-6 mt-6">
